import _ from "lodash"; //하위의 모든 자식을 리턴하는 프로그램
...
export default function findChildrens(data, id, oputArr, callback) {
  ...
  if (filterArr.length) {
    ...
    oputArr = [...oputArr, ...filterArr.map(({
      id
    }) => id)]; //Recursive call for again search next node data
...
export function getChildrenPid(collection, Pids) {
  var result = [];
  var found = false;
  Pids = [].concat(Pids);
  collection.forEach(function iterator(value) {
