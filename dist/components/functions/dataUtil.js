...
export const join = (keys, ...lists) => lists.reduce((res, list) => {
  list.forEach(record => {
...
export const highlightString = (name, replaceStr) => {
  ...
  name = name.replace(new RegExp(`(${normReq.join("|")})`, "gi"), match => "<mark>" + match + "</mark>");
  return name;
};
...
export function objectToComma(curobj1, pkey) {
  ...
  const objMerge = (pkey, key, value) => {
    let rtn;
    if (pkey) rtn = _.merge({}, allobj.findobj | {}, {
      [pkey + "." + key]: value
    });else rtn = _.merge({}, allobj.findobj | {}, {
      [key]: value
    });
    return rtn;
  };
        ...
        return obj = { ...obj,
          [vr[0]]: { ...obj[vr[0]],
            [vr[1]]: { ...obj[vr[0]][vr[1]],
              [vr[2]]: { ...obj[vr[0]][vr[1]][vr[2]],
                [vr[3]]: { ...obj[vr[0]][vr[1]][vr[2]][vr[3]],
                  [vr[4]]: { ...obj[vr[0]][vr[1]][vr[2]][vr[3]][vr[4]],
                    [k]: obj1
                  }
                }
              }
...
export const getNodefromkeyvalue = (treeDt, keyArray) => {
  let rtnArr = [];
  const subList = getChildrenPid(treeDt, ""); //leaf level nodes

  rtnArr = rtnArr.concat(_.filter(subList, o => {
...
export const getNodeData = (allData, topNode, key, parentkey, rootkey, title) => {
  // 1. convert flatarray to children style
  let treeDt = getTreeFromFlatData({
    flatData: allData.map(node => ({ ...node,
      title: node[title]
    })),
  ...
  if (topNode !== "") {
    ...
    const subList = getChildren(treeDt, topNode); //3. revconvert subList to children style

    treeDt = getTreeFromFlatData({
      flatData: subList.map(node => ({ ...node,
        title: node[title]
      })),
      getKey: node => node[key],
      getParentKey: node => node[parentkey],
