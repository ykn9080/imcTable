...
//  alasql("DROP TABLE IF EXISTS `left`");
...
export const JoinData = async (ar1, ar2, key, type) => {
  ...
  if (!type) type = "";
  //var qstr=`SELECT * FROM ? arr1 JOIN ? arr2 USING ${key}`;
...
export const Groupby = (dt, opt) => {
  ...
  //sample
  let data = [
    { Phase: "Phase 1", Step: "Step 1", Value: 12, Value1: 112 },
    { Phase: "Phase 1", Step: "Step 2", Value: 35, Value1: 135 },
    { Phase: "Phase 2", Step: "Step 1", Value: 55, Value1: 155 },
    { Phase: "Phase 2", Step: "Step 2", Value: 55, Value1: 155 },
  ];
  groupfields = ["Phase"];
  valuefields = ["Value"];
  groupbytype = "AVG";
  //qrystring sample: "SELECT Step, AVG([Value]) AS [Value] ,SUM([Value1]) AS [Value1] FROM ? GROUP BY  Step"
...
export const testGroup = (num) => {
  ...
  var qrystring =
    "SELECT Step, AVG([Value]) AS [Value] ,SUM([Value1]) AS [Value1] FROM ? GROUP BY  Step";
